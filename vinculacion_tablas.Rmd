---
title: "vinculacion_tablas"
output: html_document:
  toc: true
  toc_depth: 1
date: "2025-10-27"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
rm(list=ls())
library(dplyr)
library(lubridate)
```

```{r}
datos_calidad<-read.csv("data/rvvcca.csv",sep=";",header=TRUE)
#arreglar nombres columnas
datos_calidad <- datos_calidad %>%
  rename(
    As=As..ng.m..,
    Ni=Ni..ng.m..,
    Cd=Cd..ng.m..,
    BaP=B.a.p..ng.m..,
    Pb=Pb..ng.m..
  )


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500472023_AVFRANCIA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500472023_AVFRANCIA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.
      )%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# 1. Convertir la columna 'Fecha' de texto (AAAA-MM-DD) a formato Date
datos_calidad <- datos_calidad %>%
  mutate(Fecha = as.Date(Fecha, format = "%Y-%m-%d"))

# Agregar columna Estacion como "Av de Francia"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Avda. Francia")

datos_combinados <- bind_rows(datos_calidad, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


```
HACER LO MISMO PARA AV FRANCIA 2024 Y 2025

```{r}
#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500472024_AVFRANCIA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500472024_AVFRANCIA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))




# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.
      )%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# 1. Convertir la columna 'Fecha' de texto (AAAA-MM-DD) a formato Date
datos_calidad <- datos_calidad %>%
  mutate(Fecha = as.Date(Fecha, format = "%Y-%m-%d"))


# Agregar columna Estacion como "Av de Francia"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Avda. Francia")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# # Verificación
# # 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

```
2025
```{r}
#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500472025_AVFRANCIA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500472025_AVFRANCIA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))




# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.
      )%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# 1. Convertir la columna 'Fecha' de texto (AAAA-MM-DD) a formato Date
datos_calidad <- datos_calidad %>%
  mutate(Fecha = as.Date(Fecha, format = "%Y-%m-%d"))


# Agregar columna Estacion como "Av de Francia"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Avda. Francia")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# # 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_calidad)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```

HAY QUE HACER ESTO PARA TODAS LAS ESTACIONES Y AÑOS, CONCATENANDO LOS DATASETS HASTA LLEGAR AL ULTIMO. 
PARA CADA ESTACION HAY QUE VER QUE VARIABLES COINCIDEN Y CUALES NO.

#PISTASILLA
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500302023_PISTASILLA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500302023_PISTASILLA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Pista Silla"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Pista Silla")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500302024_PISTASILLA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500302024_PISTASILLA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Pista Silla"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Pista Silla")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500302025_PISTASILLA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500302025_PISTASILLA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Pista Silla"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Pista Silla")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))


```


#VIVEROS
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500432023_VIVEROS.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500432023_VIVEROS.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Viveros"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Viveros")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500432024_VIVEROS.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500432024_VIVEROS.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Viveros"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Viveros")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500432025_VIVEROS.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500432025_VIVEROS.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Viveros"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Viveros")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))


```
#POLITECNIC
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500462023_POLITECNIC.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500462023_POLITECNIC.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Politecnico"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Politecnico")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500462024_POLITECNIC.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500462024_POLITECNIC.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Politecnico"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Politecnico")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500462025_POLITECNIC.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500462025_POLITECNIC.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Politecnico"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Politecnico")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```
#MOLIDELSOL
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500482023_MOLIDELSOL.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500482023_MOLIDELSOL.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Moli del Sol"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Moli del Sol")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500482024_MOLIDELSOL.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500482024_MOLIDELSOL.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Moli del Sol"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Moli del Sol")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500482025_MOLIDELSOL.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500482025_MOLIDELSOL.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Moli del Sol"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Moli del Sol")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```
#CONSELLERIA
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500492023_CONSELLERIA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500492023_CONSELLERIA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Conselleria Meteo"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Conselleria Meteo")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500492024_CONSELLERIA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500492024_CONSELLERIA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Conselleria Meteo"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Conselleria Meteo")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500492025_CONSELLERIA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500492025_CONSELLERIA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Conselleria Meteo"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Conselleria Meteo")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```
#BULEVARDSUD
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500502023_BULEVARDSUD.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500502023_BULEVARDSUD.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))



# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Bulevard Sud"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Bulevard Sud")

# 1. Comando de eliminación
datos_2023 <- datos_2023 %>%
  select(-FA, -DahA, -IcdP, -BkFA, -BjFA, -BbFA, -BaA)

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500502024_BULEVARDSUD.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500502024_BULEVARDSUD.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Bulevard Sud"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Bulevard Sud")

# 1. Comando de eliminación
datos_2024 <- datos_2024 %>%
  select(-FA, -DahA, -IcdP, -BkFA, -BjFA, -BbFA, -BaA)

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500502025_BULEVARDSUD.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500502025_BULEVARDSUD.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Bulevard Sud"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Bulevard Sud")

# 1. Comando de eliminación
datos_2025 <- datos_2025 %>%
  select(-FA, -DahA, -IcdP, -BkFA, -BjFA, -BbFA, -BaA)

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```
#CENTRO
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500542023_CENTRO.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500542023_CENTRO.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Valencia Centro"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Valencia Centro")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500542024_CENTRO.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500542024_CENTRO.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Valencia Centro"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Valencia Centro")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500542025_CENTRO.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500542025_CENTRO.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))

# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Valencia Centro"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Valencia Centro")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```

#PORTMOLL
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462503012023_PORTMOLL.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462503012023_PORTMOLL.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))


# Agregar columna Estacion como "Puerto Moll Trans. Ponent"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Puerto Moll Trans. Ponent")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462503012024_PORTMOLL.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462503012024_PORTMOLL.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Puerto Moll Trans. Ponent"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Puerto Moll Trans. Ponent")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462503012025_PORTMOLL.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462503012025_PORTMOLL.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Puerto Moll Trans. Ponent"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Puerto Moll Trans. Ponent")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```
#PORTLLIT
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462503022023_PORTLLIT.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462503022023_PORTLLIT.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))


# Agregar columna Estacion como "Puerto llit antic Turia"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Puerto llit antic Turia")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462503022024_PORTLLIT.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462503022024_PORTLLIT.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Puerto llit antic Turia"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Puerto llit antic Turia")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462503022025_PORTLLIT.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462503022025_PORTLLIT.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Puerto llit antic Turia"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Puerto llit antic Turia")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```
#NAZARET
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462509002023_NAZARET.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462509002023_NAZARET.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))

# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))


# Agregar columna Estacion como "Nazaret Meteo"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Nazaret Meteo")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462509002024_NAZARET.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462509002024_NAZARET.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Nazaret Meteo"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Nazaret Meteo")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462509002025_NAZARET.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462509002025_NAZARET.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA,
    Velocidad.del.viento=Veloc.,
    Direccion.del.viento=Direc.,
    Temperatura=Temp.,
    Humidad.relativa=H.Rel.,
    Presion=Pres.,
    Radiacion.solar=R.Sol.,
    Precipitacion=Precip.,
    Velocidad.maxima.del.viento="Veloc.m\xe1x.")%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Nazaret Meteo"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Nazaret Meteo")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```
#OLIVERETA
```{r}
#guardar variable datos comnados anterior para emplear luego al hacer la suma final de filas
datos_combinados_anterior <- datos_combinados

#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2023/MDEST462500552023_OLIVERETA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2023 <- read.csv("data/2023/MDEST462500552023_OLIVERETA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2023) <- nombres
# print("NOMBRES 2023")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))

# A. Estandarizar NOMBRES 2023 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2023 <- datos_2023 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))


# Agregar columna Estacion como "Valencia Olivereta"
datos_2023 <- datos_2023 %>%
  mutate(Estacion = "Valencia Olivereta")

datos_combinados <- bind_rows(datos_combinados_anterior, datos_2023)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2024/MDEST462500552024_OLIVERETA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2024 <- read.csv("data/2024/MDEST462500552024_OLIVERETA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 

names(datos_2024) <- nombres
# print("NOMBRES 2024")
# print(names(datos_2023))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2024 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2024 <- datos_2024 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Valencia Olivereta"
datos_2024 <- datos_2024 %>%
  mutate(Estacion = "Valencia Olivereta")

datos_combinados <- bind_rows(datos_combinados, datos_2024)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())


#    Leemos solo 1 línea (nrows=1) y saltamos las 3 líneas de metadatos.
nombres <- read.csv("data/2025/MDEST462500552025_OLIVERETA.csv", 
                        sep = "\t", dec = ",", 
                        header = FALSE, 
                        skip = 3, 
                        nrows = 1)

#Leer los DATOS, saltando las 4 líneas de encabezados + unidades
#Saltamos las 3 líneas de metadatos + la línea de nombres (4) + la línea de unidades (5).
datos_2025 <- read.csv("data/2025/MDEST462500552025_OLIVERETA.csv", 
                       sep = "\t", dec = ",", 
                       header = FALSE,
                       skip = 5) 
# 
names(datos_2025) <- nombres
# print("NOMBRES 2025")
# print(names(datos_2025))
# print("NOMBRES DATOS CALIDAD")
# print(names(datos_calidad))


# A. Estandarizar NOMBRES 2025 (Datos Meteorológicos)
# Asumimos que estos nombres ya están asignados a datos_2023
# Solo vamos a renombrar las variables de meteo a nombres más claros:
datos_2025 <- datos_2025 %>%
  rename(
    Fecha=FECHA)%>%
  # 2. Convertir la columna 'Fecha' de texto (DD/MM/AAAA) a formato Date
  mutate(Fecha = dmy(Fecha))

# Agregar columna Estacion como "Valencia Olivereta"
datos_2025 <- datos_2025 %>%
  mutate(Estacion = "Valencia Olivereta")

datos_combinados <- bind_rows(datos_combinados, datos_2025)

# 2. Crear la nueva columna 'Id' secuencial
datos_combinados <- datos_combinados %>%
  # Creamos una nueva columna 'Id' que asigna un número de fila secuencial
  # a todo el conjunto de datos combinado.
  mutate(Id = row_number()) %>%
  # Opcional: Reordenar las columnas para que 'Id' sea la primera
  select(Id, everything())

# Verificación
# 1. Mira las últimas filas (deberían ser las de 2024)
# head(datos_combinados) 
# tail(datos_combinados)

#El  numero de filas de datos combinados debe ser igual a la suma de las filas de los datasets individuales
print(nrow(datos_combinados_anterior)+nrow(datos_2023)+nrow(datos_2024)+nrow(datos_2025))
print(nrow(datos_combinados))
```

# Finalmente, guardamos el conjunto de datos combinado en un archivo CSV
```{r}
write.csv(datos_combinados, "data/datos_meteorologicos_combinados.csv", row.names = FALSE)
# El argumento row.names = FALSE se utiliza para evitar que R incluya una columna adicional en el archivo CSV que contiene los números de fila (índices) del data frame. Ya están en la columna Id.

```