...
leer_datos_meteo <- function(ruta_archivo, nombre_estacion) {
  ...
  datos_archiv <- read.csv(ruta_archivo, sep="\t", dec=",", header=FALSE, skip=5, fileEncoding = "latin1")
  names(datos_archiv) <- nombres 
  # Algunos archivos traen “FECHA” en mayúsculas, otros en minúsculas.
  if ("FECHA" %in% names(datos_archiv)) {
    datos_archiv <- datos_archiv %>% rename(Fecha = FECHA)
  }
  renombres <- c(
  ...
  for (nuevo in names(renombres)) {
    ...
    if (viejo %in% names(datos_archiv)) {
      datos_archiv <- datos_archiv %>%
